editing-bold = Kalın
editing-cancel-and-lose-any-unsaved-changes = Kaydedilmemiş değişiklikler iptal edilsin mi?
editing-cant-edit-original-image-data = Düzenlenemiyor - orijinal resim bulunamadı.
editing-card-returned-to-new-card-queue = Kart yeni kart sırasına geri döndü.
editing-card-unsuspended = Kart askıdan çıkarıldı
editing-from-camera = Kameradan
editing-from-file = Dosyadan
editing-from-photos = Fotoğraftan
editing-italic = İtalik
# keyboard shortcut description
editing-next-cloze = Sıradaki Boşluk
editing-next-field = Sıradaki Alan
editing-note-deleted = (Not silindi)
editing-note-suspended = Not askıya alındı
# (Note) type: 
editing-note-type-prompt = Tür
editing-note-unsuspended = Not askıdan çıkarıldı
# keyboard shortcut description
editing-same-cloze = Benzer Boşluk
editing-discard = vazgeç
