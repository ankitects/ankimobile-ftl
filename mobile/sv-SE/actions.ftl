actions-add-new = Lägg Till Ny
actions-answer-easy = Svara Lätt
actions-are-you-sure = Är du säker?
actions-bottom-bar = Bottenraden
actions-browse = Bläddra
actions-card-buried = Kortet är gömt
actions-card-suspended = Kort låst
actions-card-template = Kortmallar
actions-confirm-delete = Bekräfta Ta bort
actions-create = Skapa
actions-deck-statistics = Kortlek Statistik
actions-discard-changes = Kasta Ändringar
actions-done = Klar
actions-draw = Rita
actions-marked = Märkt
actions-tools = Verktyg
actions-top-bar = Toppraden
actions-undo = Ångra

## Short actions. These will be shown in the top bar, and should be kept to about
## 5-7 characters where possible.

actions-add-short = Lägg till
actions-answer-again-short = Igen
actions-answer-easy-short = Lätt
actions-answer-good-short = Bra
actions-answer-hard-short = Svårt
actions-card-template-short = Mall
