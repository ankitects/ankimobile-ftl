## iOS permissions

info-plist-camera-usage = Questo permette Anki di inserire le immagini della fotocamera nelle tue carte.
info-plist-mic-usage = Il microfono verrà solo usato quando clicchi il pulsante di registrazione.
info-plist-photo-usage = Questo permette di mostrare il browser delle foto.
info-plist-photo-add-usage = L'azione "Aggiungi alle Foto" richiede il tuo permesso.

## Long-press app icon shortcuts

info-plist-add-cards = Aggiungi carta
info-plist-search = Cerca

## File imports

info-plist-packaged-collection = Collezione Anki Impacchettata
info-plist-packaged-anki-deck = Mazzo Anki Impacchettato
